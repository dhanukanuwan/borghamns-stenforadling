...
const imagesOne = require.context('../images/step-8/one', true);
...
const StepEight = () => {
    ...
    useEffect(() => {
        ...
        const savedHo = localStorage.getItem('borg_step_8_hall_type');
    ...
    const handleHoSelect = (val) => {
        setHoType(val);
        localStorage.setItem('borg_step_8_hall_type', val);
    }

    return(
        <div className="step-wrap h-100 position-relative">
             <h2 className="h3 liten">{ __('Välj häll-typ', 'borghamns-general' ) }</h2>
